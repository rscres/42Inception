...
then
	...
	echo "CREATE DATABASE ${MYSQL_DB};" >> ${TMP}
	echo "CREATE USER '${WP_ADMIN_N}'@'%' IDENTIFIED BY '${WP_ADMIN_P}';" >> ${TMP}
	echo "GRANT ALL PRIVILEGES ON ${MYSQL_DB}.* TO '${WP_ADMIN_N}'@'%' IDENTIFIED BY '${WP_ADMIN_P}';" >> ${TMP}
	echo "FLUSH PRIVILEGES;" >> ${TMP}
